...
class TestEmbeddingGradients:
    def test_token_embedding_gradients(self):
        ...
        loss.backward()
        assert x.grad is not None or x_real.grad is not None or x_imag.grad is not None, "Gradients not flowing through token embedding"
        # At least one of the real or imag grads should be nonzero
    ...
    def test_data_embedding_gradients(self):
        ...
        loss = output.real.sum()
        loss.backward()
        # Check gradients for real and imag parts
        assert x_real.grad is not None or x_imag.grad is not None, "Gradients not flowing through data embedding"
        assert (x_real.grad is not None and torch.any(x_real.grad != 0)) or (x_imag.grad is not None and torch.any(x_imag.grad != 0)), "Gradients are all zero"
